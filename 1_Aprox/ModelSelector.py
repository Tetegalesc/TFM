...
class ModelSelector():
    ...
    def CreateLabel(self, umb=0.04):
        ...
        plt.savefig('Results/0_Data/Results/Gain_umb.png')
        plt.close()
        ...
        plt.savefig('Results/0_Data/Results/Label.png')
        plt.close()
    ...
    def TrainTestSplit(self):
        ...
        plt.plot(self.X_train['close'], label='Train')
        plt.plot(self.X_test['close'], label='Test')
        plt.plot(self.X_val['close'], label='Val')
        ...
        plt.savefig('Results/0_Data/Results/Train_Test_Split.png')
        plt.close()
        ...
        for i, (train_index, test_index) in enumerate(self.cv.split(self.X_train)):
            axs[i].plot(self.X_train['close'], color = 'k')
            axs[i].plot(self.X_train['close'][train_index], label = 'Train')
            axs[i].plot(self.X_train['close'][test_index], label = 'Test')
            axs[i].yaxis.set_visible(False)

        plt.savefig('Results/0_Data/Results/CV_Folds.png')
        plt.close()
    ...
    def FeatureSelection(self, model_name):
        ...
        print('Searching Best Features...')

        wrapping = SFS(model, k_features=1, forward=False, floating=False, scoring=self.metric, cv=self.cv, n_jobs=-1, verbose=2)

        wrapping = wrapping.fit(self.X_train, self.y_train)
        ...
        for i in wrapping.subsets_.keys():
            if wrapping.subsets_[i]['avg_score'] > best_feat_score:
                best_feat_score = wrapping.subsets_[i]['avg_score']
                best_feat_wrapping = list(wrapping.subsets_[i]['feature_names'])
                best_num_feat = i

        self.best_feat = best_feat_wrapping

        print('El mejor rendimiento obtenido es {rend}, usando los atributos: \n{atribut} \nNúmero de atributos usados : {num}'
              .format(rend = best_feat_score, atribut = best_feat_wrapping, num = best_num_feat))
        ...
        # Saving Results
        feature_selection_results_path = 'Results/2_Feature_Selection/Feature_Selection_Results.txt'
    ...
    def HyperparametersTuning(self, model_name, features, grid):
        ...
        def objective(params):
            ...
            elif model_name == 'BagSVM':
                model = BaggingClassifier(estimator=SVC(**params))
            elif model_name == 'XGB':
        ...
        self.best_hyper = hyperopt.fmin(fn = objective, space = self.hyper_space, algo = hyperopt.tpe.suggest, max_evals = 500, trials = trials,
